...
# - DNS for staging domains points to this VPS (or EXPECTED_IP)
# - Key staging URLs respond with 200/expected status
# - Browser console check for dashboard URLs (no JS errors)
#
# Usage:
#   ./BACKEND/scripts/check-staging-postdeploy.sh
#   EXPECTED_IP=72.60.202.91 ./BACKEND/scripts/check-staging-postdeploy.sh
#   SKIP_BROWSER_CHECK=1 ./BACKEND/scripts/check-staging-postdeploy.sh
...
echo "=== STAGING POST-DEPLOY CHECKS ==="
echo ""
...
echo "2) HTTP status check for key staging URLs"
echo "----------------------------------------"

urls=(
  "https://staging.fastpaygaming.com/login"
  "https://sredpay.fastpaygaming.com/login"
  "https://sapi.fastpaygaming.com/health/"
)
...
if [[ $failed -ne 0 ]]; then
  echo "One or more staging URLs failed. Treat this deploy as FAILED."
  exit 1
fi
...
echo ""
echo "All staging DNS, HTTP, and browser checks passed."
