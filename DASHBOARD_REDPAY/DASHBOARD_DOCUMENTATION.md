...
**Staging vs production:** Use `DASHBOARD/.env.staging` for staging builds and `.env.production` for production. Set `VITE_API_BASE_URL` to the backend API origin for each environment. For Gmail/Drive OAuth on a domain, see **BACKEND/docs/GMAIL_DRIVE_DEPLOY.md** (backend and dashboard env + Google Console).
