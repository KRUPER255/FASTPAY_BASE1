...
function CreateUserDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      // Company is automatically set to REDPAY in the API call
      ...
      toast({ title: 'Success', description: 'User created successfully for REDPAY company' })
      reset()
      onOpenChange(false)
      onSuccess()
    } catch (e) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new REDPAY dashboard user. They can log in with email and password.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-3 bg-muted rounded-md">
            <p className="text-sm text-muted-foreground">
              <strong>Company:</strong> REDPAY (automatically assigned)
            </p>
          </div>
          <DialogFooter>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {user && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-3 bg-muted rounded-md">
              <p className="text-sm text-muted-foreground">
                <strong>Company:</strong> {user.company_name || user.company_code || 'REDPAY'} (cannot be changed)
              </p>
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>
...
export function UserManagementSection() {
  ...
  const loadUsers = async () => {
    ...
    setLoading(true)
    try {
      // Backend automatically filters to REDPAY company for RedPay users
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <CardTitle>REDPAY Users</CardTitle>
            <CardDescription>Create and manage REDPAY dashboard users and their access levels.</CardDescription>
          </div>
      ...
      <CardContent>
            ...
            <TableBody>
              {users.map((u) => (
                <TableRow key={u.email}>
                  ...
                  <TableCell>
                    <Badge variant="default">{u.company_name || u.company_code || 'REDPAY'}</Badge>
                  </TableCell>
        ...
        {!loading && users.length === 0 && (
          <p className="text-muted-foreground text-center py-8">No REDPAY users yet. Create one to get started.</p>
        )}
      </CardContent>
